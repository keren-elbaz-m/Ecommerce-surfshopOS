---
description: Inject relevant standards into the current context, auto-suggested or explicitly selected
argument-hint: "[folder or folder/file ...]"
---
...
2. If not found: tell the user this repo isn't onboarded — run the installer (`project-install.sh`) or `/plan-customer` first. Stop.
3. Standards always live in the LOCAL repo's `softwareos/standards/` — `customer_root` does not apply to standards.
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `softwareos/standards/` (not in a subfolder).
...
Read `softwareos/standards/index.yml` to get the list of available standards and their descriptions.
...
@softwareos/standards/api/response-format.md
@softwareos/standards/api/error-handling.md
@softwareos/standards/global/naming.md
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `softwareos/standards/api/`
- **Folder/file** — `api/response-format` → inject `softwareos/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `softwareos/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `softwareos/standards/naming.md`
