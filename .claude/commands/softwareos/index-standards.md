---
description: Rebuild and maintain the standards index (index.yml) used by /inject-standards
---
...
1. List all `.md` files in `softwareos/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `softwareos/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `softwareos/standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `softwareos/standards/index.yml` with this structure:
...
**Rules:**
- `root` first, then remaining folders alphabetized; files alphabetized within each folder
...
**Note:** `root` appears first and contains standards files that live directly in `softwareos/standards/` (not in subfolders).
...
Updates `softwareos/standards/index.yml`
